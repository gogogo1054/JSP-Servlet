...
public class MemberAuth extends HttpServlet
{
	...
	/*
	 	클라이언트가 최초로 요청했을때 서블릿객체가 생성되는데 이때 최초 1번만
	 	호출되는 init() 메서드에서 DB 연결처리를 한다.
	 */
	@Override
	public void init() throws ServletException 
	{
		/*
		 	서블릿 내에서 application 내장 객체를 얻어온다. 모델2 방식에서는
			...
			throws ServletException, IOException 
	{
		...
		// 회원이름을 통해 로그인 성공여부를 판단한다. 판단 후 출력할 메세지 이름을 request 영역에 저장한다.
		...
		else
		{
			...
			else
			{
				...
				// 만약 최고관리자도 아니라면 비회원이라 판단할 수 있다.
				req.setAttribute("authMessage", "귀하는 회원이 아닙니다.");
			}
		}
		// 처리할 메세지를 request영역에 저장한 후 JSP로 포워드 한다.
		req.getRequestDispatcher("/13Servlet/MemberAuth.jsp").forward(req, resp);
	}

	// 서블릿 종료 시 DAO객체도 같이 지원해제 한다.
	@Override
	public void destroy() {
		dao.close();
	}
