...
public class FrontController extends HttpServlet
{
	...
	/*
	 	get방식으로 들어오는 과정을 처리하기 위해 doGet()메서드를 오버라이딩해야한다.
	 	만약 서블릿이 이 메서드가 정의되지 않았다면 405에러가 발생하게 된다. 즉 해당
	 	방식의 요청을 처리할 수 없다는 의미이다.
	 */
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		// 마지막 요청명을 통해 요청을 판단한 후 해당 요청을 처리할 메서드를 호출한다. 이때
		// 사용자의 요청정보를 담은 request 객체를 인수로 전달한다. 즉 모든 요청이
